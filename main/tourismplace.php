...
require 'Models/tourismplace.php';
    ...
    if ($result != NULL) {
        ...
        if ($count == 0){
            ...
            while ($row= $result->fetch_assoc()) {
				echo '<a href="tourismplace.php/'.$row["id"].'"><div class="col-12 col-sm-6 col-lg-4">
						<div class="single-features-area mb-50">
							<img src="http://'.$row["PHOTOURL"].'" alt="">
							<div class="feature-content d-flex align-items-center justify-content-between">
								...
								<div class="feature-favourite">
									<a href="tourismplace.php/'.$row["id"].'"><i class="fa fa-plus-o" aria-hidden="true"></i></a>
								</div>
							</div>
						</div>
					</div></a>';
...
$app->post('/',function() use ($app){
	...
	$tourismplace= new tourismplace();
    $result = $tourismplace->distance($long,$lat,$distance);
     if ($result != NULL) {
        $response["error"] = false;
        $count = $result->num_rows;
        if ($count == 0){
...
$app->get('/:id', function($id) use ($app) {
	$tourismplace = new tourismplace();
    $result = $tourismplace->singlefetch($id);
    if ($result != NULL) {
        $response["error"] = false;
        $count = $result->num_rows;
        if ($count == 0){
            ...
            while ($row= $result->fetch_assoc()) {
	echo '<div class="breadcumb-area height-600 bg-img bg-overlay" style="background-image: url(http://'.$row["PHOTOURL"].')">
	<div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="breadcumb-content">
                        <div class="map-ratings-review-area d-flex">
                            <a href="../tourismplace.php">Tourism Place</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row justify-content-center">
                ...
                <div class="addthis_inline_share_toolbox"></div>
						...
						<h4>Brief Description</h4>
                            <p>'.$row["DESCRIPTION"].' '.$row["Description2"].'</p>
							<p>Opening Hours: '.$row["Opening_Hours"].'</p>
							<p>For more info, visit:<br /><a href="http://'.$row["URL_Path"].'">http://'.$row["URL_Path"].'</a></p>
                        </div>
